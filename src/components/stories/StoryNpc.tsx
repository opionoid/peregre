...
export const StoryNpc: React.VFC<IStoryNpcProps> = (props) => {
  ...
  const handleClick = (skill: ISkill) =>
    onClickSkill(skill, setDepth, props.name)
  ...
  return (
    <StoryNpcWrapper>
      <Info>
        <h4>{props.name}</h4>
        <Status>
          <label aria-label="魂魄量">
            <Icon src={Icons.MaxHp} />
          </label>
          <HpInput value={hp} onChange={(e) => setHp(Number(e.target.value))} />
        </Status>
        <Status>
          <label aria-label="魂魄量">
            <Icon src={Icons.MaxDepth} />
          </label>
          <span>{depth}</span>
        </Status>
        <ToggleButton>
          <ButtonBase aria-controls="skills" onClick={() => toggleShown()}>
            {shouldShowSkills ? 'スキル非表示' : 'スキル表示'}
          </ButtonBase>
        </ToggleButton>
      </Info>
        ...
        {props.skills.map((skill) => (
          <Skill key={skill.name}>
            ...
            <SkillButton>
              <ButtonBase
                onClick={() =>
                  handleClick({ ...skill, icon: { src: '' }, isUlt: false })
                }
              >
                使用
              </ButtonBase>
